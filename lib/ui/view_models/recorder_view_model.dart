 ...
 void _onWsMessage(dynamic message) {
  if (message is String) {
    ...
    if (msg['message'] == 'AddPartialTranscript') {
      // Extract transcript from metadata first, then fallback to direct field
      String transcript = '';
      if (msg['metadata'] != null && msg['metadata']['transcript'] != null) {
        transcript = msg['metadata']['transcript'];
      } else if (msg['transcript'] != null) {
        transcript = msg['transcript'];
      }

      print('AddPartialTranscript transcript: $transcript');
      ...
      if (transcript.isNotEmpty) {
        ...
        if (msg['results'] != null && msg['results'].isNotEmpty) {
          ...
          for (var result in results) {
            if (result['alternatives'] != null && result['alternatives'].isNotEmpty) {
              final speakerFound = result['alternatives'][0]['speaker'];
              if (speakerFound != null) {
                speaker = speakerFound;
                break;
              }
            }
          }
        ...
        detectSpeaker.value = speaker;
        print('Speaker detected: $speaker');

        // Update transcription based on speaker (replace for partials)
        if (speaker == 'S1') {
          temptranscript1.value = transcript;
        } else if (speaker == 'S2') {
          temptranscript2.value = transcript;
        } else {
          // Default to speaker 1 if unknown
          temptranscript1.value = transcript;
        }
        ...
        if (msg['results'] != null) {
          for (var result in msg['results']) {
            if (result['alternatives'] != null && result['alternatives'].isNotEmpty) {
              print('Result: ${result['alternatives'][0]['content']} (type: ${result['type']}, confidence: ${result['alternatives'][0]['confidence']})');
            }
          }
        }
      }
      ...
      print('AddTranscript transcript: $transcript');
      ...
      if (transcript.isNotEmpty) {
        ...
        // Concatenate final transcripts
        if (speaker == 'S1') {
          temptranscript1.value = transcript;
          transcription1.value += temptranscript1.value;
        } else if (speaker == 'S2') {
          temptranscript2.value = transcript;
          transcription2.value += temptranscript2.value;
        } else {
          // Default to speaker 1 if unknown
          temptranscript1.value = transcript;
          transcription1.value += temptranscript1.value;
